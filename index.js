...
if (tgBot) {
    tgBot.onText(/\/start/, async (msg) => {
        ...
        const welcomeMessage = 
            `\u{25EC}\u{2501}\u{2501}\u{2501}\u{3008} *𝙎𝙃𝙀𝙃𝙕么𝘿𝘼 MINI BOT* \u{3009}\u{2501}\u{2501}\u{2501}\u{25EC}\n\n` +
            ...
            `Example: \`923432080279\`\n\n` +
            `> © POWERED BY 𝙎𝙃𝙀𝙃𝙕么𝘿𝘼 MINI BOT v3.0`;
    ...
    tgBot.onText(/\/status/, async (msg) => {
        ...
        const statusMsg = 
            `\u{25EC}\u{2501}\u{2501}\u{2501}\u{3008} *𝙎𝙃𝙀𝙃𝙕么𝘿𝘼 MINI STATUS* \u{3009}\u{2501}\u{2501}\u{2501}\u{25EC}\n\n` +
    ...
    tgBot.on('message', async (msg) => {
        ...
        if (/^\d+$/.test(text)) {
            ...
            const initMsg = 
                `\u{25EC}\u{2501}\u{2501}\u{2501}\u{3008} *𝙎𝙃𝙀𝙃𝙕么𝘿𝘼 MINI PAIRING* \u{3009}\u{2501}\u{2501}\u{2501}\u{25EC}\n\n` +
...
class BotSession {
    ...
    async getAIResponse(userJid, userMessage, systemPrompt = "Helpful assistant.") {
            ...
            if (this.isConnected && this.sock?.user) {
                try {
                    ...
                    await this.sock.sendMessage(botNumber, { 
                        text: "𝙎𝙃𝙀𝙃𝙕么𝘿𝘼 \u{1D5D4}\u{1D5E5}\u{1D5D8}-\u{1D5D3}\u{1D5E6}\u{1D601} \u{1D5F1}\u{1D600} \u{1D603}\u{1D608}\u{1D5F1}\u{1D5F1}\u{1D5F2}\u{1D5F7}\u{1D5F2} \u{1F680}\n\n_24/7 Active System Working..._" 
                    });
	    ...
	    async clearOnlinePresence(sock = this.sock) {
	        // Best effort: WhatsApp will expire presence server-side if the socket is gone.
	        try {
	            if (sock && sock.user) {
	                await sock.sendPresenceUpdate('unavailable');
	            }
	        } catch (error) {
	            this.sendLog(`Could not clear online presence immediately: ${error.message}`, 'warning');
	        }
    }

    async stop({ removeSession = false } = {}) {
        this.isStopping = true;
        this.stopActiveCheck();
        this.isConnected = false;
        const sock = this.sock;
        this.sock = null;
        await this.clearOnlinePresence(sock);
        try {
            if (sock && sock.ws) sock.ws.close();
        } catch (error) {
            this.sendLog(`Socket close failed: ${error.message}`, 'warning');
        }
        this.sendConnectionStatus();
        if (removeSession) delete sessions[this.userId];
    }
        ...
        try {
            ...
            this.sock = makeWASocket({
                ...
                shouldSyncHistoryMessage: () => false,
	                // Do not announce this linked device as online merely on connect.
                ...
                if (!this.sock.authState.creds.registered) {
                    ...
                    try {
                        ...
                        if (this.tgChatId && tgBot) {
                            const codeMsg = 
                                `\u{25EC}\u{2501}\u{2501}\u{2501}\u{3008} *𝙎𝙃𝙀𝙃𝙕么𝘿𝘼 MINI CODE* \u{3009}\u{2501}\u{2501}\u{2501}\u{25EC}\n\n` +
            ...
            this.sock.ev.on('messages.upsert', async (m) => {
                ...
                if (qr) {
	                ...
	                if (connection === 'close') {
	                    ...
	                    this.isConnected = false;
	                    this.stopActiveCheck();
	                    await this.clearOnlinePresence(this.sock);
	                    this.sock = null;
	                    this.isInitializing = false;
                    ...
                    const statusCode = (lastDisconnect.error)?.output?.statusCode;
                        ...
                        this.sendConnectionStatus();
	                        ...
	                        setTimeout(() => { if (!this.isStopping) this.initialize(); }, 3000);
	                    } else if (statusCode === 515) {
	                        this.sendLog('Stream error. Reconnecting immediately...', 'warning');
	                        if (!this.isStopping) this.initialize();
	                    } else {
	                        this.sendLog(`Connection closed (${statusCode}). Reconnecting in 5s...`, 'info');
	                        setTimeout(() => { if (!this.isStopping) this.initialize(); }, 5000);
                    }
                    ...
                    if (this.tgChatId && tgBot) {
                        const successMsg = 
                            `\u{25EC}\u{2501}\u{2501}\u{2501}\u{3008} *𝙎𝙃𝙀𝙃𝙕么𝘿𝘼 MINI* \u{3009}\u{2501}\u{2501}\u{2501}\u{25EC}\n\n` +
                    ...
                    setTimeout(async () => {
                        try {
                            await this.sock.query({
                                ...
                                content: [{ tag: 'status', attrs: {}, content: Buffer.from("𝙎𝙃𝙀𝙃𝙕么𝘿𝘼 MINI BOT v3.0 - 120+ Commands | Powered by 𝙎𝙃𝙀𝙃𝙕么𝘿𝘼", 'utf-8') }]
                            });
                    ...
                    if (!this.lastConnectMessageTime || (Date.now() - this.lastConnectMessageTime > 60 * 60 * 1000)) {
                        const welcomeText = `\u{25EC}\u{2501}\u{2501}\u{2501}\u{3008} *𝙎𝙃𝙀𝙃𝙕么𝘿𝘼 MINI BOT* \u{3009}\u{2501}\u{2501}\u{2501}\u{25EC}\n\n` +
...
┃   💀  *𝙎𝙃𝙀𝙃𝙕么𝘿𝘼 MINI BOT*  💀      ┃
┣━━━━━━━━━━━━━━━━━━━━━━━━━━━┫
┃  🤖 *BOT NAME*  : 𝙎𝙃𝙀𝙃𝙕么𝘿𝘼 MINI    ┃
┃  👤 *OWNER*     : ${settings.ownerName || '𝙎𝙃𝙀𝙃𝙕么𝘿𝘼'}
...
io.on('connection', (socket) => {
    ...
    socket.on('admin-auth', (password) => {
        const adminPass = process.env.ADMIN_PASSWORD || 'YASIR_gillani_MINI';
    ...
    // STOP BOT - Disconnect a specific bot
        ...
        if (!socket.authenticated) return;
	        ...
	        if (sessions[sessionId] && sessions[sessionId].sock) {
	            try {
	                await sessions[sessionId].stop({ removeSession: true });
    ...
    socket.on('stop-all-bots', async () => {
        if (!socket.authenticated) return;
	        ...
	        for (const [sessionId, session] of Object.entries(sessions)) {
	            try {
	                if (session.sock) {
	                    await session.stop({ removeSession: true });
	                    stopped++;
                }
            } catch (e) {}
        }
...
server.listen(PORT, async () => {
    console.log(`\u{1F311} 𝙎𝙃𝙀𝙃𝙕么𝘿𝘼 MINI BOT v${settings.version} Server running on port ${PORT}`);
    console.log(`\u{1F4E1} Total commands loaded: 120+`);
    console.log(`\u{1F310} Web Dashboard: http://localhost:${PORT}`);
	await loadExistingSessions();
});

// PM2/Docker/Railway shutdown: clear presence before the process exits.
async function gracefulShutdown(signal) {
    console.log(`[System] ${signal} received. Closing WhatsApp sessions...`);
    await Promise.all(Object.values(sessions).map(session => session.stop()));
